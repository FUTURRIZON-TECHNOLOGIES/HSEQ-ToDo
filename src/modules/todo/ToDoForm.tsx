...
const buildFormData = (item: IToDoItem | undefined): IToDoItem => {
    ...
    return {
        ...
        TrainingInductionId: item.TrainingInductionId != null ? item.TrainingInductionId : item.TrainingInduction?.Id,
    };
};
...
const ToDoForm: React.FC<IToDoFormProps> = ({ item, spService, context, onSave, onRefresh, onClose }) => {
    ...
    const [options, setOptions] = React.useState<{
        ...
        trainingInduction: IDropdownOption[];
    }>({ status: [], category: [], classification: [], priority: [], regarding: [], trainingInduction: [] });
    ...
    const fetchAttachments = React.useCallback(async () => {
        if (item?.Id) {
            try { setAttachments(await spService.getAttachments("ToDo", item.Id)); }
        ...
        const loadOptions = async () => {
            ...
            try {
                ...
                setOptions({
                    ...
                    trainingInduction: (await spService.getLookupOptions('TrainingInductions', 'Title')).map(o => ({ key: o.Id, text: o.Title })),
                });
            } catch (e) {
    ...
    const handleSaveInternal = async (mode: 'stay' | 'close' | 'new') => {
        ...
        const getInternal = (displayName: string, fallback: string) =>
            spService.getInternalName("ToDo", displayName, fallback);
        ...
        const trainingInternal = getInternal('Training & Induction', 'TrainingInduction');
        payload[`${trainingInternal}Id`] = formData.TrainingInductionId;
    ...
    const handleUpload = async (file: File) => {
        if (item?.Id) {
            await spService.uploadAttachment("ToDo", item.Id, file);
    ...
    if (item?.Id && confirm(`Delete "${fileName}"?`)) {
        await spService.deleteAttachment("ToDo", item.Id, fileName);
        await fetchAttachments();
    }
};
                    ...
                    <div className={styles.fieldRow}>
                        ...
                        <div className={styles.fieldHalf}>
                            <Dropdown
                                ...
                                options={options.trainingInduction}
                                selectedKey={formData.TrainingInductionId}
                                onChange={(_, opt) => setFormData({ ...formData, TrainingInductionId: opt?.key as number })}
